...
const MiscellaneousSection = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validation: Check if name or about is empty
    if (!name.trim() || !about.trim()) {
      setFeedback({
        message: "Name and About fields cannot be empty.",
        color: "red",
      });
  ...
  return (
    <div className="w-screen h-full flex justify-center items-start overflow-auto p-6 bg-gray-100 rounded-lg">
      <div className="w-5/6 bg-white p-6 rounded-md shadow-md">
        ...
        {feedback.message && (
          <div
            className={`py-2 w-full bg-green-100 text-center text-xl  text-${feedback.color}-600`}
          >
            {feedback.message}
          </div>
        )}
        ...
        <div>
          <form onSubmit={handleSubmit}>
            ...
            <div className="overflow-auto flex justify-center text-center items-center px-2 py-2 w-full">
              <input
                ...
                className={`w-full md:w-3/4 text-center text-2xl md:text-4xl font-semibold border rounded-lg p-2 ${
                  !name.trim() && feedback.message.includes("Name")
                    ? "border-red-500"
                    : ""
                }`}
              />
              ...
              <div className="overflow-auto px-2 py-2 w-full">
                ...
                <textarea
                  ...
                  className={`w-full h-32 text-lg border rounded-lg p-2 resize-none ${
                    !about.trim() && feedback.message.includes("About")
                      ? "border-red-500"
                      : ""
                  }`}
                />
            ...
            {/* Boundaries Section */}
            <div className="py-5">
              <h2 className="py-2 font-bold text-3xl md:text-4xl text-center">
                Map Boundaries
              </h2>

  ...
  const [isSidebarOpen, setIsSidebarOpen] = useState(!!location.state?.selectedIssue); 
    ...
    const handleOutsideClick = (event) => {
        if (selectedMarker || newMarker) {
            ...
            if (mapRef.current && mapRef.current.getContainer().contains(event.target)) {
                return;
            }
            setSelectedMarker(null);
            setNewMarker(null);
            ...
            <div className="h-full flex mt-4">
                ...
                <div className={`h-full flex flex-col ${selectedMarker || newMarker ? 'w-4/6 hidden sm:flex' : 'flex-grow min-w-0'} relative`}>
                    ...
                    <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2 z-10 mb-10">
                        <button className={`px-4 py-2 rounded-lg border-2 border-gray-400 ${filterToggles.open ? 'bg-blue-500 text-white border-blue-500' : 'bg-gray-200'}`} onClick={() => handleToggleChange('open')}>
                            Unresolved
                        </button>
                        <button className={`px-4 py-2 rounded-lg border-2 border-gray-400 ${filterToggles.resolved ? 'bg-green-500 text-white border-green-500' : 'bg-gray-200'}`} onClick={() => handleToggleChange('resolved')}>
                            Resolved
                        </button>
                        <button className={`px-4 py-2 rounded-lg border-2 border-gray-400 ${filterToggles.closed ? 'bg-red-500 text-white border-red-500' : 'bg-gray-200'}`} onClick={() => handleToggleChange('closed')}>
                            Closed
                        </button>
                    </div>
                </div>
